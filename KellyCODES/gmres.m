...
while((rho > errtol) & (k < kmax))
    k=k+1; % line (a)
    v(:,k+1)=feval(atv,v(:,k));  %compute Av_k
    normav=norm(v(:,k+1));
    %
    % Modified Gram-Schmidt
    %
    %     Subtract from Av_k along the direction of each of the already
    %     obtained basis vectors
    for j=1:k %  
        h(j,k)=v(:,j)'*v(:,k+1); % line (b) - project v_j onto Av_k
        v(:,k+1)=v(:,k+1)-h(j,k)*v(:,j); % line (c)  -  subtract
    end
    h(k+1,k)=norm(v(:,k+1)); %line (d)
    normav2=h(k+1,k);
    %
    % Reorthogonalize?
    %
