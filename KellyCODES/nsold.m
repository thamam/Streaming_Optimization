function [sol, it_hist, ierr,outstat] = nsold(x,f,tol,parms, options,varargin)
...
%     outstat(itc+1, :) = [itc fnrm rat];
    ...
    if(itc == 1 | rat > rsham | itsham == 0)
        itsham=isham;
        if options.UseHessian
            useFBS = 1; % forward-backward solver
            if useFBS 
                [step, l, u] =   ONA_FBS(f0, H0, options.bksz);
            else
                [l,u] =   lu(H0);
                tmp = -l\f0;
                step = u\tmp;
            ...
            %
    % compute the step using same Hessian factorization - only gradient is
    % different - this is the Shamanski approach 
    %
    tmp = -l\f0;
    step = u\tmp;       
    end
...
%     while  nft >= (1 - alpha*lambda) * nf0
    while ( (nft >= (1 - alpha*lambda) * nf0) | sum(xt<0) )
        %
...
%             it_hist=it_histx(1:itc+1,:);
    ...
    %% Verify that xt is feasible . i.e. xt>=0          
%     c=1e-2;
%     if sum(xt<0 & 1)
%        [val, ind] = min(xt);
%        a = (-val+step(ind))/step(ind);
%        xt = x+(1-c)*a*step;       
%     end
